		...
		<v-row>
			<v-col cols="10" class="scroll-container">
				<div v-for="(chatContent, index) in chatContents" v-bind:key="index">
					<div v-if="chatContent.sendMessageType === 'JOIN'">
						<v-row>
							<v-col />
							<v-col cols="3">
								<v-card>
					...
					<div v-if="chatContent.sendMessageType === 'LEAVE'">
						<v-row>
							<v-col />
							<v-col cols="3">
								<v-card>
					...
					<div v-else-if="chatContent.sendMessageType === 'TALK'">
						<v-row v-if="chatContents[index].id != user.state.id && (index == 0 || chatContents[index].id != chatContents[index - 1].id)" style="height: 30px">
						...
						<v-row v-if="chatContents[index].id == user.state.id">
							...
							<v-col :style="{ fontSize: '10px', display: 'grid', justifyContent: 'end', gridTemplateColumns: 'auto auto' }" cols="9">
								<span v-if="index == chatContents.length - 1 || chatContents[index].id != chatContents[index + 1].id || formatTimeChat(chatContents[index].time) != formatTimeChat(chatContents[index + 1].time)" :style="{ fontSize: '10px', alignSelf: 'end', width: '35px' }" class="font-weight-bold">{{ formatTimeChat(chatContent.time) }}</span>
						...
						<v-row v-if="chatContents[index].id != user.state.id">
							<v-col :style="{ display: 'grid', gridTemplateColumns: '40px auto 35px', justifyContent: 'start', justifyItems: 'end' }" cols="9">
								...
								<span v-if="index == chatContents.length - 1 || chatContents[index].id != chatContents[index + 1].id || formatTimeChat(chatContents[index].time) != formatTimeChat(chatContents[index + 1].time)" :style="{ fontSize: '10px', display: 'grid', alignItems: 'end' }" class="font-weight-bold">{{ formatTimeChat(chatContent.time) }} </span>
							</v-col>
						</v-row>
					</div>
				</div>
...
import { onMounted, reactive, ref, watchEffect, nextTick, watch, onUnmounted, onBeforeUnmount, onBeforeMount } from 'vue';
...
const handleSubscribeMessage = message => {
	...
	console.log('chatUsers: ', chatUsers.value);
};
...
watchEffect(() => {
	...
	stompClient.value = new Client({
		...
		onConnect: () => {
			...
			stompClient.value.subscribe('/topic/' + moviesId.value, handleSubscribeMessage);
		},
...
watch(chatUsers, () => {
	const currentUserIndex = chatUsers.value.findIndex(chatUser => chatUser.usersId == user.state.id);
...
const handleChatInput = event => {
	...
	stompClient.value.publish({
		...
		body: JSON.stringify({ content: chatContent.value }),
	});
	chatContent.value = '';
};
</script>
